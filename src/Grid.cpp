    ...
    num_p_cells(p_width * p_height)
{
    ...
    for (uint32_t i = 0; i < particles.size(); i++) {
        ...
        particles[i] = {static_cast<float>(f_cell_size * 2) + x + RADIUS * (y % 2), static_cast<float>(f_cell_size * 2) + (y * test), 0.0f};
    }
    particleVelToCell();
    updateDensity();
    //particles[0] = {static_cast<float>(f_cell_ratio) + RADIUS + 10, static_cast<float>(f_cell_ratio) + RADIUS};
}
...
inline void Grid::pBounds(Particle& p) {
    p.x_pos = std::clamp(p.x_pos,DIAMETER * f_cell_size + RADIUS,static_cast<float>(p_width) - (DIAMETER * f_cell_size + RADIUS));
    p.y_pos = std::clamp(p.y_pos,DIAMETER * f_cell_size + RADIUS,static_cast<float>(p_height) - (DIAMETER * f_cell_size + RADIUS));
}
...
const std::optional<std::reference_wrapper<const Cell>> Grid::getCell(const uint32_t x, const uint32_t y) {
...
void Grid::setCellField(float Cell::*field, const float value, const uint32_t x, const uint32_t y) {
    if (x < f_width && y < f_height)
...
void Grid::updateVAs() {
    for (uint32_t j = 0; j < f_height; j++) {
        for (uint32_t i = 0; i < f_width; i++) {
            ...
            line_vertices[line_va_idx + 1].position = sf::Vector2f(x0 + f_grid[pos].x_vel / 15, y);
            line_vertices[line_va_idx + 2].position = sf::Vector2f(x, y0);
            line_vertices[line_va_idx + 3].position = sf::Vector2f(x, y0 + f_grid[pos].y_vel / 15);
        }
    }
...
inline const Grid::cellData Grid::getData(Particle& p, const bool mx, const bool my) {
    const float dx = mx ? 0.0f : RADIUS * f_cell_size;
    const float dy = my ? 0.0f : RADIUS * f_cell_size;
    // const float dx = 0.0f;
    // const float dy = 0.0f;
    const float x = std::clamp(p.x_pos, static_cast<float>(f_cell_size), static_cast<float>((f_width - 1) * f_cell_size));
    const float y = std::clamp(p.y_pos, static_cast<float>(f_cell_size), static_cast<float>((f_height - 1) * f_cell_size));
    const uint32_t x0 = std::min(static_cast<uint32_t>((x - dx) / f_cell_size), f_width - 2);
    const uint32_t y0 = std::min(static_cast<uint32_t>((y - dy) / f_cell_size), f_height - 2);
...
void Grid::particleVelToCell() {
    ...
    for (uint32_t j = 0; j < f_height; j++) {
        for (uint32_t i = 0; i < f_width; i++) {
            ...
            if (f_grid[f_pos].getCellType() == Cell_Type::SOLID || (i > 0 && f_grid[f_pos - 1].getCellType() == Cell_Type::SOLID))
                f_grid[f_pos].x_vel = f_grid[f_pos].p_x_vel;
            if (f_grid[f_pos].getCellType() == Cell_Type::SOLID || (j > 0 && f_grid[f_pos - f_width].getCellType() == Cell_Type::SOLID))
