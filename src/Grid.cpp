...
Grid::Grid(const uint32_t width, const uint32_t height, const uint32_t f_cell_ratio_, const uint32_t num_particles_, const uint32_t render_scale_):
    f_width(width),
    f_height(height),
    f_cell_ratio(f_cell_ratio_),
    num_f_cells(f_width * f_height),
    num_particles(num_particles_),
    render_scale(render_scale_),
    p_width(width * f_cell_ratio_),
    p_height(height * f_cell_ratio_),
    num_p_cells(p_width * p_height)
{
    ...
    for (uint32_t j = 0; j < f_height; j++) {
        for (uint32_t i = 0; i < f_width; i++) {
            ...
            const uint32_t scale = render_scale * f_cell_ratio * DIAMETER;
            ...
            f_cell_vertices[cell_va_idx + 2].position = sf::Vector2f(x + scale, y + scale);
    ...
    for (uint32_t i = 0; i < particles.size(); i++) {
        ...
        const uint32_t x = i % 100;
        const uint32_t y = i / 100;
        ...
        particles[i] = {static_cast<float>(f_cell_ratio * 2) + x + RADIUS * (y % 2), static_cast<float>(f_cell_ratio * 2) + (y * test), 0.0f};
    }
}
...
inline const uint32_t Grid::P2F(const uint32_t x, const uint32_t y) const {
    return y / f_cell_ratio * f_width + x / f_cell_ratio;
}
...
inline void Grid::pBounds(Particle& p) {
    p.x_pos = std::clamp(p.x_pos,DIAMETER * f_cell_ratio + RADIUS,static_cast<float>(p_width) - (DIAMETER * f_cell_ratio + RADIUS));
    p.y_pos = std::clamp(p.y_pos,DIAMETER * f_cell_ratio + RADIUS,static_cast<float>(p_height) - (DIAMETER * f_cell_ratio + RADIUS));
}
...
void Grid::setField(const Field field, const float value, const uint32_t x, const uint32_t y) {
    ...
    {
    case Field::X_VEL:
        f_grid[getFPos(x,y)].x_vel = value;
        break;
    case Field::Y_VEL:
        f_grid[getFPos(x,y)].y_vel = value;
        break;
    default:
        break;
    }
...
void Grid::updateVAs() {
    for (uint32_t j = 0; j < f_height; j++) {
        for (uint32_t i = 0; i < f_width; i++) {
            ...
            line_vertices[line_va_idx + 1].position = sf::Vector2f(x0 + f_grid[pos].x_vel, y);
            line_vertices[line_va_idx + 2].position = sf::Vector2f(x, y0);
            line_vertices[line_va_idx + 3].position = sf::Vector2f(x, y0 + f_grid[pos].y_vel);
        }
    }
...
void Grid::project() {
    ...
    for (uint32_t k = 0; k < NUM_F_SUB_STEP; k++) {
        for (uint32_t f_pos = 0; f_pos < num_f_cells; f_pos++) {
            ...
            const uint8_t s =   f_grid[f_pos + 1].getS() + 
                                f_grid[f_pos - 1].getS() + 
            ...
            float d =   f_grid[f_pos + 1].x_vel - 
                        f_grid[f_pos].x_vel + 
                        f_grid[f_pos + f_width].y_vel - 
                        f_grid[f_pos].y_vel;
...
inline const std::array<bool,4> Grid::getValidArr(const std::array<uint32_t,4> f_pos, const uint32_t offset) {
    return std::array<bool,4>{
        f_grid[f_pos[0]].getCellType() != Cell_Type::AIR || f_grid[f_pos[0] - offset].getCellType() != Cell_Type::AIR,
        f_grid[f_pos[1]].getCellType() != Cell_Type::AIR || f_grid[f_pos[1] - offset].getCellType() != Cell_Type::AIR,
        f_grid[f_pos[2]].getCellType() != Cell_Type::AIR || f_grid[f_pos[2] - offset].getCellType() != Cell_Type::AIR,
        f_grid[f_pos[3]].getCellType() != Cell_Type::AIR || f_grid[f_pos[3] - offset].getCellType() != Cell_Type::AIR,
    };
}

inline const float Grid::getPic(float Cell::* feild, const std::array<uint32_t,4> f_pos, const std::array<float,4> vd, const float vd_s) {
    return  (vd[0] * f_grid[f_pos[0]].*feild +
            vd[1] * f_grid[f_pos[1]].*feild +
            vd[2] * f_grid[f_pos[2]].*feild +
            vd[3] * f_grid[f_pos[3]].*feild) / vd_s;
}

inline const float Grid::getFlip(float Cell::* feild, float Cell::* pre_feild, const std::array<uint32_t,4> f_pos, const std::array<float,4> vd, const float vd_s) {
    return  (vd[0] * (f_grid[f_pos[0]].*feild - f_grid[f_pos[0]].*pre_feild) +
            ...
            vd[3] * (f_grid[f_pos[3]].*feild - f_grid[f_pos[3]].*pre_feild)) / vd_s;
}

inline const Grid::cellData Grid::getData(Particle& p, const bool x, const bool y) {
    const float dx = x ? 0.0f : RADIUS * f_cell_ratio;
    const float dy = y ? 0.0f : RADIUS * f_cell_ratio;
    // const float dx = 0.0f;
    // const float dy = 0.0f;
    const uint32_t x0 = std::clamp(static_cast<uint32_t>((p.x_pos - dx) / f_cell_ratio), 0u, f_width - 1);
    const uint32_t y0 = std::clamp(static_cast<uint32_t>((p.y_pos - dy) / f_cell_ratio), 0u, f_height - 1);
    const uint32_t x1 = std::clamp(x0 + 1, 0u, f_width - 1);
    const uint32_t y1 = std::clamp(y0 + 1, 0u, f_height - 1);

    const float dx_0 = ((p.x_pos - dx) - (x0 * f_cell_ratio)) / f_cell_ratio;
    const float dy_0 = ((p.y_pos - dy) - (y0 * f_cell_ratio)) / f_cell_ratio;
...
void Grid::cellVelToParticle() {
    for (Particle& p : particles) {
        ...
        if (vdx_s != 0.0f) {
            ...
            const float flip_x_vel = getFlip(&Cell::x_vel, &Cell::p_x_vel, cdx.f_pos, vdx, vdx_s);
        ...
        if (vdy_s != 0.0f) {
            ...
            const float flip_y_vel = getFlip(&Cell::y_vel, &Cell::p_y_vel, cdy.f_pos, vdy, vdy_s);
...
void Grid::particleVelToCell() {
    ...
    for (uint32_t f_pos = 0; f_pos < num_f_cells; f_pos++) { 
        f_grid[f_pos].x_vel = r_x[f_pos] == 0.0f ? 0.0f : f_grid[f_pos].x_vel / r_x[f_pos];
        f_grid[f_pos].y_vel = r_y[f_pos] == 0.0f ? 0.0f : f_grid[f_pos].y_vel / r_y[f_pos];
        if (f_grid[f_pos].getCellType() == Cell_Type::SOLID || (f_pos > 0 && f_grid[f_pos - 1].getCellType() == Cell_Type::SOLID))
            f_grid[f_pos].x_vel = f_grid[f_pos].p_x_vel;
        if (f_grid[f_pos].getCellType() == Cell_Type::SOLID || (f_pos > f_width && f_grid[f_pos - f_width].getCellType() == Cell_Type::SOLID))
...
void inline Grid::collideSolid(const uint32_t pos1, const uint32_t pos2) {
    if (pos2 < num_p_cells) {
        ...
        for (uint32_t i = 0; i < v1l; i++) {
            ...
            const uint32_t x = (pos2 % p_width) / f_cell_ratio;
            const uint32_t y = (pos2 / p_width) / f_cell_ratio;
            const uint32_t f_pos2 = getFPos(x,y);
            if (f_grid[f_pos2].getCellType() == Cell_Type::SOLID) {
                const uint32_t x0 = x * f_cell_ratio;
                const uint32_t y0 = y * f_cell_ratio;
                const uint32_t x1 = (x + 1) * f_cell_ratio;
                const uint32_t y1 = (y + 1) * f_cell_ratio;
                const float closest_x = std::clamp(p.x_pos, static_cast<float>(x0), static_cast<float>(x1));
                const float closest_y = std::clamp(p.y_pos, static_cast<float>(y0), static_cast<float>(y1));
