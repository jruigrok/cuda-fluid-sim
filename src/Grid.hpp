...
public:
	...
	Grid(const uint32_t width, const uint32_t height, const uint32_t f_cell_ratio, const uint32_t num_particles, const uint32_t scale);
...
private:
	...
	const std::array<bool,4> getValidArr(const std::array<uint32_t,4> f_pos, const uint32_t offset);
	const float getPic(float Cell::* feild, const std::array<uint32_t,4> f_pos, const std::array<float,4> vd, const float vd_s);
	const float getFlip(float Cell::* feild, float Cell::* pre_feild, const std::array<uint32_t,4> f_pos, const std::array<float,4> vd, const float vd_s);
	...
	static constexpr float GRAVITY = 10.0f * 0.15;
	static constexpr float DT = (1.0 / 165.0) / 0.15;
	...
	static constexpr uint32_t NUM_P_SUB_STEP = 50u;
	...
	static constexpr uint32_t NUM_F_SUB_STEP = 20u;
	const uint32_t f_width;
	const uint32_t f_height;
	const uint32_t num_f_cells;
	const uint32_t f_cell_ratio;
