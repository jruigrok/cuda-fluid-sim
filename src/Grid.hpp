...
public:
	...
	const std::optional<std::reference_wrapper<const Cell>> getCell(const uint32_t x, const uint32_t y);
	virtual void draw(sf::RenderTarget& target, sf::RenderStates states) const;
	void setCellField(float Cell::*field, const float value, const uint32_t x, const uint32_t y);
