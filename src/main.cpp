...
int main(int argc, char *argv[]) {
  ...
  sf::RenderWindow window(sf::VideoMode(SCREEN_WIDTH, SCREEEN_HEIGHT),
                          "Fluid Sim");
  ...
  while (window.isOpen()) {
    ...
    if (viewPort.getMouseDown()) {
      ...
      grid.setCellType(Cell_Type::SOLID,
                       static_cast<uint32_t>(mouse_pos.x / F_CELL_RATIO),
                       static_cast<uint32_t>(mouse_pos.y / F_CELL_RATIO));
